"""Context Retriever module for the Contextual Post Explainer agent.

Formulates search queries from post content and retrieves relevant
web context using the Tavily Search API.
"""
...
from src.post_extractor import PostContent
...
class ContextRetriever:
    ...
    def formulate_queries(self, post: PostContent) -> list[str]:
        ...
        if post.mentions and len(queries) < _MAX_QUERIES:
            # Mentions are DIDs, which aren't useful as search queries on their own
            # Combine with text snippet for context
        ...
        # Fallback: if text was empty but we have hashtags, they're already in queries.
        # If we still have nothing (shouldn't happen for non-empty text per Property 6),
    ...
    async def search(self, post: PostContent) -> list[SearchResult]:
        ...
        Returns:
            ...
            Returns empty list if all queries return zero results.
